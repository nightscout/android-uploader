...
public class AndroidBarcodeTest extends RobolectricTestBase {
  ...
  private void setSingleValidApiAndMongoWithIntentResult() {
    jsonConfig =
        "{'mongo':{'uri':'mongodb://user:pass@test.com/cgm_data'}, 'rest':{'endpoint':['http://abc@test.com/']}}";
    fakeActivityResult();
  }
  ...
  private void fakeActivityResult() {
    ...
    new SettingsActivity()
        .onActivityResult(IntentIntegrator.REQUEST_CODE, Activity.RESULT_OK, intent);
  }
  ...
  public void validMongoOnlyShouldSetDefaultSgCollectionForOnlyMongoUriSet() {
    setValidMongoOnlyWithIntentResult();
    assertThat(prefs.getMongoCollection(), is(getShadowApplication().getApplicationContext()
                                                  .getString(
                                                      R.string.pref_default_mongodb_collection)));
  }

  @Test
  public void validMongoOnlyShouldSetDefaultDeviceStatusCollectionForOnlyMongoUriSet() {
    setValidMongoOnlyWithIntentResult();
    assertThat(prefs.getMongoDeviceStatusCollection(),
