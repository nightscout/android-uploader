...
//        downloadObject.deDup();
    ...
    protected void checkUploaderBattery(){
        ...
        if (downloadObject.getUploaderBattery() < UPLOADERBATTERYCRITICAL){
            downloadObject.addMessage(new AlertMessage(AlertLevel.CRITICAL, "Uploader battery " + verb + " critically low: " + (int) downloadObject.getUploaderBattery(), Conditions.UPLOADERCRITICALLOW));
        }else if (downloadObject.getUploaderBattery() < UPLOADERBATTERYWARN) {
            downloadObject.addMessage(new AlertMessage(AlertLevel.WARN, "Uploader battery "+verb+" low: "+(int) downloadObject.getUploaderBattery(),Conditions.UPLOADERLOW));
        }
    }
    ...
    protected void checkCGMBattery(){
        if (downloadObject.getStatus()== DownloadStatus.SUCCESS) {
            ...
            if (downloadObject.getDeviceBattery() < DEVICEBATTERYCRITICAL) {
                downloadObject.addMessage(new AlertMessage(AlertLevel.CRITICAL, "CGM battery " + verb + " critically low" + downloadObject.getDeviceBattery(), Conditions.DEVICECRITICALLOW));
            }else if (downloadObject.getDeviceBattery() < DEVICEBATTERYWARN) {
                downloadObject.addMessage(new AlertMessage(AlertLevel.WARN, "CGM battery "+verb+" low: "+downloadObject.getDeviceBattery(),Conditions.DEVICELOW));
            }
        }
    }
    ...
    protected void checkRecordAge(){
        ...
        try {
            ...
            if (recordAge > MAXRECORDAGE && downloadAge <= MAXRECORDAGE) {
                ...
                downloadObject.addMessage(new AlertMessage(AlertLevel.CRITICAL, "CGM out of range/missed reading for " + Utils.getTimeString(downloadObject.getLastRecordReadingDate(), new Date()), Condition.MISSEDREADING));
//                downloadObject.addMessage(new AlertMessage(AlertLevels.CRITICAL, "CGM out of range or missed reading for more than " + ((recordAge / 1000) / 60) + " minutes old", Conditions.MISSEDREADING));
            ...
            downloadObject.addMessage(new AlertMessage(AlertLevel.WARN,"Download did not contain any data",Condition.NODATA));
        }
        if (downloadAge > MAXRECORDAGE)
            downloadObject.addMessage(new AlertMessage(AlertLevel.CRITICAL,"Uploader inactive for "+ Utils.getTimeString(downloadObject.getDownloadDate(), new Date()),Condition.STALEDATA));
    }
    ...
    protected void checkThresholdholds(){
        try {
            ...
            String preamble=(downloadObject.getConditions().contains(Condition.MISSEDREADING) || downloadObject.getCondition().contains(Condition.STALEDATA))?"Last reading: ":"";
    ...
    protected void checkLastRecordTime(){
        String msg;
        try {
            msg=Utils.getTimeString(downloadObject.getLastRecordReadingDate(), new Date());
