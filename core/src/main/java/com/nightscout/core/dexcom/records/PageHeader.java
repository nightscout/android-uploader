...
public class PageHeader {
  ...
  public PageHeader(byte[] packet) {
    if (packet.length < HEADER_SIZE) {
      try {
        throw new InvalidRecordLengthException(
            "Data smaller than expected: " + packet.length + ". Expected size: " + HEADER_SIZE
    ...
    firstRecordIndex =
        ByteBuffer.wrap(packet).order(ByteOrder.LITTLE_ENDIAN).getInt(FIRSTRECORDINDEX_OFFSET);
    ...
    if (!Arrays.equals(this.crc, crc_calc)) {
      throw new CRCFailError(
          "CRC check failed: " + Utils.bytesToHex(this.crc) + " vs " + Utils.bytesToHex(crc_calc));
    }
