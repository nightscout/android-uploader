...
public class Utils {
  ...
  private static final PeriodFormatter FORMATTER = new PeriodFormatterBuilder()
      ...
      .appendSeparator(PRIMARY_SEPARATOR, SECONDARY_SEPARATOR)
  ...
  /**
   * Returns human-friendly string for the length of this duration, e.g. 4 seconds ago or 4 days
   * ago.
   *
  ...
  public static GlucoseDataSet[] mergeGlucoseDataRecords(EGVRecord[] egvRecords,
    ...
    for (int i = 1; i <= smallerLength; i++) {
      glucoseDataSets[smallerLength - i] =
          new GlucoseDataSet(egvRecords[egvLength - i], sensorRecords[sensorLength - i]);
    }
